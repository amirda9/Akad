                                ...
                                @if($order->is_approved)
                                    ...
                                    @else
                                        <div class="bg-white rounded shadow-sm p-3 text-secondary mb-3">
                                            @if($order->isReserved())
                                                <h4>سفارش رزرو</h4>
                                                <p>سفارش شما در حالت رزرو می باشد، لطفا برای پرداخت و ارسال سفارش، ابتدا خرید خود را تکمیل کنید.</p>
                                                <a class="btn btn-success btn-block btn-lg" href="{{ route('user.orders.done',$order->code) }}">
                                                    تکمیل خرید
                                                </a>
                                            @else
                                                <form method="post" action="{{ route('orders.pay',$order->code) }}">
                                                    ...
                                                    @foreach(\App\Order::payments() as $key => $payment)
                                                        <label id="payment-{{$key}}" class="d-flex align-items-center border rounded mb-3 p-2 shadow-md-hover cursor-pointer">
                                                            ...
                                                            <div class="mr-2">
                                                                <div><strong>{{ $payment['title'] }}</strong></div>
                                                                <small class="m-0">{{ $payment['description'] }}</small>
                                                            </div>
                                                        </label>
                                                    @endforeach
                                ...
                                <div class="bg-white rounded shadow-sm p-3 text-secondary mb-3" style="font-size: 18px;">
                                    ...
                                    @else
                                        @if(!$order->is_paid)
                                            <div class="d-flex justify-content-between align-items-center flex-wrap mb-3">
                                                <span>کد تخفیف</span>
                                                <form action="{{route('orders.submitCoupon',$order->code)}}" method="post">
                                                    @csrf
                                                    <div class="form-row align-items-center">

                                                    <div class="col-auto">
                                                        <label class="sr-only" for="inlineFormInputGroup">کد تخفیف</label>
                                                        <div class="input-group mb-2">
                                                        <input type="text" class="form-control" name="code" id="inlineFormInputGroup" placeholder="کد تخفیف">
                                                        </div>
                                                    </div>

                                                    <div class="col-auto">
                                                        <button type="submit" class="btn btn-primary mb-2">اعمال کد تخفیف</button>
                                                    </div>
                                                    </div>
                                                </form>
                                            </div>

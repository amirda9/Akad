    ...
    <div class="container py-3">
        ...
        <div class="row">
            <div class="col-12 col-lg-3 mb-3 mb-lg-0">
                <form id="filters" style="top: 55px;" method="GET" enctype="multipart/form-data">
                    <input type="hidden" id="orderby-input" name="orderBy" value="{{ request('orderBy') }}">
                    <div class="card shadow-sm mb-3">
                        <div class="card-header bg-gray-50">
                            <h6 class="m-0 text-secondary">دسته بندی نتایج</h6>
                        </div>
                        <div class="card-body p-1 category-list-container">
                            @foreach($parents as $pr)
                                <a href="{{ $pr->getRoute() }}" class="category-list-link pr-level-{{ $pr->level }}">
                                    <i class="fa fa-angle-left ml-1"></i>
                                    {{ $pr->name }}
                                </a>
                            @endforeach
                            <a href="{{ $category->getRoute() }}" style="font-weight: 600" class="category-list-link pr-level-{{ $category->level }}">
                                @if($category->children()->count())
                                    <i class="fa fa-angle-down ml-1"></i>
                                @endif
                                {{ $category->name }}
                            </a>
                            @foreach($category->children()->orderBy('order','asc')->get() as $item)
                                <a href="{{ $item->getRoute() }}" class="category-list-link pr-level-{{ $item->level }}">
                                    {{ $item->name }}
                                </a>
                            @endforeach
                        </div>

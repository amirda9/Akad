    ...
    <div class="content-wrapper">
        ...
        <div class="content">
            <div class="container-fluid">
                <div class="card">
                    <div class="card-body">
                        <form method="POST" action="{{ route('panel.slides.update',$slide) }}" enctype="multipart/form-data">
                            @csrf
                            @method('put')
                            <div class="form-group row">
                                <label for="image" class="col-sm-4 text-sm-left col-form-label text-nowrap">تصویر جدید اسلاید</label>
                                <div class="col-sm-5">
                                    <input
                                            type="file"
                                ...
                                <div class="col-sm-5">
                                    <select name="position" data-placeholder='انتخاب موقعیت نمایش'
                                        ...
                                        @foreach(\App\Slide::$positions as $position_index => $position_title)
                                            <option {{ $position_index == $slide->position ? 'selected' : '' }} value={{ $position_index }}>{{$position_title}}</option>
                                        @endforeach
                                    </select>

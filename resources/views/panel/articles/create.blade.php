    ...
    <div class="content-wrapper">
        ...
        <div class="content">
            <div class="container-fluid">
                ...
                <form method="POST" action="{{ route('panel.posts.articles.store') }}" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="input-group mb-3">
                                        ...
                                        <select class="form-control {{ $errors->has('can_comment') ? 'is-invalid' : '' }}" name="can_comment">
                                            <option value="1" {{ old('can_comment') === '1' ? 'selected' : '' }}>فعال</option>
                                            <option value="0" {{ old('can_comment') === '0' ? 'selected' : '' }}>غیرفعال</option>
                                            <option value="" {{ old('can_comment') === null ? 'selected' : '' }}>پیش فرض</option>
                                        </select>
                                    </div>
                                    <div class="input-group mb-3">
                                        <div class="input-group-prepend">
                                        ...
                                        <select class="form-control {{ $errors->has('can_rate') ? 'is-invalid' : '' }}" name="can_rate">
                                            <option value="1" {{ old('can_rate') === '1' ? 'selected' : '' }}>فعال</option>
                                            <option value="0" {{ old('can_rate') === '0' ? 'selected' : '' }}>غیرفعال </option>
                                            <option value="" {{ old('can_rate') ===  null ? 'selected' : '' }}>پیش فرض </option>
                                        </select>
                                    </div>
...
@section('scripts')
    ...
    var options = {
        ...
        $('body').on('keydown','#tag_input',function (e) {
            ...
            renderTags();
        }).on('submit','#createProductForm',function (e) {
            ...
            if($('input[name=form_validated]').val() != 1) {
                ...
                $.ajax({
                    type: 'POST',
                    url: "{{ route('panel.products.createValidation') }}",
                    data: formData,
                    cache:false,
                    contentType: false,
                    processData: false,

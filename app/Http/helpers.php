...
if (!function_exists('getOption')) {
    function getOption($option_name, $fallback = null)
    {
        ...
        $option = Cache::remember("options_$option_names[0]", 60 * 24, function () use ($option_names) {
            $option =  \App\Option::where('name', $option_names[0])->first();
...
if (!function_exists('getProductCategoriesList')) {
    ...
    {
        if ($category == null) {
            $categories = \App\ProductCategory::where('parent_id', null)->orderBy('order', 'asc')->get();
...
if (!function_exists('sendSms')) {
    function sendSms($data, $template, $mobile)
    {
        try {
            $code = config("minooSms.templates.$template", 0);
            $apiKey = config("minooSms.api-key");
            $line_number = config("minooSms.line-number");
            ...
            \Illuminate\Support\Facades\Log::error($log);
        }
    }
}
...
if (!function_exists('render_widget_position')) {
    function render_widget_position($position_name)
    {
        $option_widgets = Cache::remember("options_widgets", 60 * 24, function () {
            $option = \App\Option::where('name', 'widgets')->first();
            if ($option) {
        ...
        foreach ($option_widgets as $widget) {
            $widget_class = new \App\Option::$widgets[$widget['name']]((array) $widget);
...
if (!function_exists('count_widget_position')) {
    function count_widget_position($position_name)
    {
        $option_widgets = Cache::remember("options_widgets", 60 * 24, function () {
            $option = \App\Option::where('name', 'widgets')->first();
            if ($option) {
        ...
        $option_widgets = collect($option_widgets);
        return $option_widgets->where('position', $position_name)->count();
    }
}
...
if (!function_exists('getAvailableShippings')) {
    function getAvailableShippings($order_price, $province = null, $city = null)
    {
        ...
        $shipping_option = \App\Option::where('name', 'shipping')->first();
        ...
        if ($shippings['free_shipping']['is_active'] ?? false) {
            ...
            if ($province == $site_province['name']) {
        ...
        if ($show_other_shipping) {
            if ($shippings['post_shipping']['is_active'] ?? false) {
                ...
                $has_bike_shipping = $shippings['bike_shipping']['is_active'] ?? false;
                if ($city != $site_city['name'] || !$has_bike_shipping) {
                    if ($province == $site_province['name']) {
